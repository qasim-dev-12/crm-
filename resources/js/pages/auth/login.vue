          ...
          <div class="container">
            <div class="row">
              <div class="col-lg-10 col-xl-7 mx-auto">
                ...
                <p class="text-muted mb-4 mt-2">{{ $t("Please login to continue your session!") }}</p>
                <form @submit.prevent="login" @keydown="form.onKeydown($event)">
                  ...
                  <div class="form-group mb-3">
                    <input v-model="form.password" :class="{ 'is-invalid': form.errors.has('password') }"
                      ...
                      name="password" :placeholder="$t('Enter your password')" />
    ...
    async login() {
      try {
        ...
        if (data.status === "otp_required") {
          localStorage.setItem("otp_token", data.__token);
          localStorage.setItem("otp_expiration_time", data.otp_expiration_time || 2);
          this.$router.push({ name: "otp_verification" });
        } else if (data.token) {
        ...
        // Handle server or network errors, including invalid credentials
        this.loginError =
