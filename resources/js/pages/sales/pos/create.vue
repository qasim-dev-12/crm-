        ...
        <div>
          <div class="row" v-if="accounts &&
            ...
            <div class="form-group col-md-6">
              <label for="paidAmount">{{ $t("Invoice Total") }}</label>
              <input ref="paidAmountInput" id="paidAmount" v-model.number="form.netTotal" type="number" step="any"
                class="form-control" :class="{ 'is-invalid': form.errors.has('paidAmount') }"
                name="paidAmount" min="1" :max="form.netTotal" :placeholder="$t('Enter an amount')" readonly>
              <has-error :form="form" field="paidAmount" />
            </div>
            <div class="form-group col-md-6">
              <label for="paidAmount">{{ $t("Paid Amount") }}<span class="required">*</span></label>
              <input ref="paidAmountInput" id="paidAmount" v-model.number="form.paidAmount" type="number" step="any"
                class="form-control" :class="{ 'is-invalid': form.errors.has('paidAmount') }"
                name="paidAmount" min="1" :max="form.netTotal" :placeholder="$t('Enter an amount')" @input="calculateChange"/>
              <has-error :form="form" field="paidAmount" />
            </div>
            <div class="form-group col-md-6">
              <label for="receivedAmount">{{ $t("Received") }}<span class="required">*</span></label>
              <input id="receivedAmount" v-model.number="form.receivedAmount" type="number" step="any"
                class="form-control" name="receivedAmount"
                :placeholder="$t('Enter amount received')" @input="calculateChange" required/>
                <has-error :form="form" field="receivedAmount" />
            </div>

            <div class="form-group col-md-6">
              <label for="changeAmount">{{ $t("Change") }}</label>
              <input id="changeAmount" v-model="form.changeAmount" type="number" step="any"
                class="form-control" name="changeAmount" readonly />
            </div>
          </div>
          <div class="row">
            <div class="form-group col-md-6">
          ...
          <div style="max-width: 400px; margin: 0px auto">
            ...
            <table class="table_data">
              <tbody>
                ...
                <tr>
                  <td colspan="3" class="total">{{ $t("Received") }}</td>
                  <td style="text-align: right" class="total">
                    {{ allData.invoicePayments[0]?.receivedAmount ?? 0 | withCurrency }}
                  </td>
                </tr>
                <tr>
                  <td colspan="3" class="total">{{ $t("Change") }}</td>
                  <td style="text-align: right" class="total">
                    {{ allData.invoicePayments[0]?.changeAmount ?? 0 | withCurrency }}
                  </td>
                </tr>
                <tr>
...
export default {
  ...
  methods: {
    ...
    calculateChange() {
      if (this.form.receivedAmount && this.form.paidAmount) {
        let change = parseFloat(this.form.receivedAmount) - parseFloat(this.form.paidAmount);
        this.form.changeAmount = change > 0 ? change.toFixed(2) : 0;
      } else {
