                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="(data, i) in items" :key="i">
                      ...
                      <td>{{ data.symbol }}</td>
                      <td>{{ data.position }}</td>
                      <td> <span> {{ formatCurrencyPreview(data) }} </span> </td>
                      <td>
                        <span
                          v-if="data.status === 1"
  ...
  methods: {
    ...
    getSeparators(format) {
      switch (format) {
        ...
        default: return { thousand: ",", decimal: "." };
      }
    },
    ...
    formatCurrencyPreview(currency) {
      const { thousand, decimal } = this.getSeparators(currency.format || "en_US");
      let exampleNumber = this.formatExample(1234567.891, currency.decimal_places, thousand, decimal);
      return currency.position === "left" ? `${currency.symbol}${exampleNumber}` : `${exampleNumber}${currency.symbol}`;
    },
  },
};
</script>
